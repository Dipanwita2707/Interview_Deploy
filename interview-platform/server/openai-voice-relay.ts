...
// Use a dedicated realtime deployment if set, otherwise fall back to gpt-4o-realtime-preview.
// AZURE_OPENAI_DEPLOYMENT is typically a non-realtime model (e.g. gpt-4.1).
...
// Build a proper wss:// URL, stripping any trailing slash from the endpoint.
const _azureEndpointBase = AZURE_ENDPOINT.replace(/\/+$/, "").replace(/^https?:\/\//, "");
const OPENAI_WS_URL = `wss://${_azureEndpointBase}/openai/v1/realtime?model=${AZURE_DEPLOYMENT}`;
...
function buildSystemPrompt(ctx: InterviewContext, startIdx: number): string {
  ...
  const examSummary = Array.isArray(ctx.examContext?.submissionSummary)
    ...
    : [];
  const hasExamContext = examSummary.length > 0;
  const examItems = examSummary.slice(0, 8).map((s, i) => {
    const preview = (s.submittedAnswerExcerpt || "").replace(/\s+/g, " ").trim();
    return `${i + 1}. ${s.questionTitle} | verdict=${s.verdict} | score=${s.score} | topics=${(s.topics || []).join(",") || "n/a"}`
      + (s.language ? ` | language=${s.language}` : "")
      + (preview ? `\n   answer_excerpt: ${preview.slice(0, 350)}` : "");
  }).join("\n");

  const examFocusBlockZh = hasExamContext
    ? `\n## 编程考试个性化上下文（必须遵守）
- 本场面试必须严格围绕候选人刚完成的编程题和其提交答案展开，不要提问与这些题无关的泛化问题。
- 优先考察：解题思路、算法选择、时间/空间复杂度、边界条件、优化点、替代方案。
- 每道题至少追问一次“为什么这么做”与“复杂度权衡”。
- 如果 verdict/score 显示有不足，要追问失败原因与修正方案。
- 如果答案正确，也要追问可扩展性与进一步优化。
- 允许改写问题措辞，但必须锚定到下面的真实做题记录。
- 禁止偏离到无关的一般性行为题。
${ctx.examContext?.examScore !== undefined ? `- 考试总分：${ctx.examContext.examScore}` : ""}
${ctx.examContext?.weakTopics?.length ? `- 薄弱主题：${ctx.examContext.weakTopics.join("，")}` : ""}
${examItems ? `- 做题记录：\n${examItems}` : ""}
`
    : "";

  const examFocusBlockEn = hasExamContext
    ? `\n## Personalized coding-exam context (mandatory)
- This interview MUST be strictly grounded in the participant's completed coding questions and submitted answers.
- Do NOT ask unrelated generic questions outside these solved problems.
- Prioritize: approach reasoning, algorithm choice, time/space complexity, edge cases, trade-offs, optimization, and alternatives.
- For each problem, probe at least "why this approach" and "complexity trade-offs".
- If verdict/score indicates issues, probe likely root causes and how they would fix them.
- If accepted/high-score, still probe scalability and optimization.
- You may rewrite wording, but keep it anchored to the solved-question records below.
- Avoid unrelated behavioral/general chatter.
...
${examItems ? `- Solved-question records:\n${examItems}` : ""}
`
    : "";
...
2. 对每个问题，你必须进行恰好${maxFollowUps}次追问，即使受访者的第一个回答看起来已经很完整——不要在第一次回答后就立即调用 signal_question_change。请深入探究：追问具体例子、细节、原因或结果。语气要像友好、耐心的真人面试官。
3. 只有在完成了${maxFollowUps}次追问并且受访者给出了详细、具体的回答之后，才能调用 signal_question_change 进入下一题。"讨论充分"是指受访者对你的追问也给出了具体回答——不是模糊的表述如"我遇到过很多挑战"。如果追问后回答仍然模糊，再追问一次，然后再切换。
...
2. For each question, you MUST ask exactly ${maxFollowUps} follow-up question(s) before moving on — even if the first answer seems complete. Do NOT call signal_question_change after the participant's very first reply. Always probe deeper: ask for examples, specifics, reasoning, or outcomes. Sound like a warm, patient human interviewer.
3. Only after you have completed ${maxFollowUps} follow-up exchange(s) AND the participant has given detailed, specific answers, call signal_question_change to advance. "Sufficiently discussed" means the participant has given concrete answers to your follow-up questions — NOT vague statements like "I had many challenges". If their answers are still vague after follow-ups, probe once more before transitioning.
...
async function handleInterview(browserWs: WebSocket, ctx: InterviewContext) {
  ...
  // Track how many user reply turns happened on the current question.
  // Enforces follow-up depth — the AI must complete at least minFollowUps
  // exchanges before signal_question_change is accepted.
  let userTurnsThisQuestion = 0;
  const minFollowUps = (() => {
    switch (ctx.followUpDepth) {
      case "LIGHT":    return 1;
        ...
        switch (msg.type) {
          ...
          case "response.function_call_arguments.done": {
            if (msg.name === "signal_question_change") {
              ...
              // not enough time has passed, insufficient words, or not enough
              // follow-up turns for the configured followUpDepth.
              // Backward transitions and explicit user requests bypass these guards.
              ...
              const needsFollowUpGuard = isForward && !userRequested;
              const followUpInsufficient = needsFollowUpGuard && userTurnsThisQuestion < minFollowUps;
              if (newIdx < sortedQuestions.length && (lastUserInput === 0 || dwellMs < MIN_QUESTION_DWELL_MS || (needsWordGuard && userCommittedWordsThisQuestion < MIN_WORDS_BEFORE_TRANSITION) || followUpInsufficient)) {
                const reason = lastUserInput === 0
                  ? "the participant has not spoken yet"
                  : followUpInsufficient
                    ? `only ${userTurnsThisQuestion} reply turn(s) on this question — you must ask at least ${minFollowUps} follow-up(s) before moving on (interview depth: ${ctx.followUpDepth})`
                ...
                pendingFunctionCalls.push({
                  callId: msg.call_id,
                  name: msg.name,
                  args: `Rejected: ${reason}. You MUST ask follow-up question(s) on question ${currentQuestionIndex + 1} before calling signal_question_change. Do NOT verbally say you are moving to another question — the transition was blocked. Continue the current question.`,
                });
                break;
              }
