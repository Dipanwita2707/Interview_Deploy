...
function buildSystemText(ctx: InterviewContext): string {
  const base = bt(isChineseInterview(ctx), SPOKEN.systemText(ctx.aiName, ctx.aiTone.toLowerCase()));
  const examSummary = Array.isArray(ctx.examContext?.submissionSummary)
    ? ctx.examContext!.submissionSummary!
    : [];
  if (examSummary.length === 0) return base;
  const isZh = isChineseInterview(ctx);
  const header = isZh
    ? `\n[个性化考试背景（强制要求）] 本次面试必须围绕考生已解答的编程题展开，逐题追问解题思路、时间/空间复杂度、权衡取舍及改进方案，禁止提问通用问题。\n`
    : `\n[PERSONALIZED EXAM CONTEXT (mandatory)] This interview must focus on the participant's solved coding problems. Probe approach, complexity, trade-offs, and improvements — NO generic questions.\n`;
  const scoreLine = ctx.examContext?.examScore !== undefined
    ? (isZh ? `考试得分: ${ctx.examContext.examScore}\n` : `Exam score: ${ctx.examContext.examScore}\n`)
    : "";
  const weakLine = ctx.examContext?.weakTopics?.length
    ? (isZh ? `薄弱知识点: ${ctx.examContext.weakTopics.join(", ")}\n` : `Weak topics: ${ctx.examContext.weakTopics.join(", ")}\n`)
    : "";
  const rows = examSummary.slice(0, 8).map((s, i) => {
    const p = (s.submittedAnswerExcerpt || "").trim();
    return `${i+1}. ${s.questionTitle} | verdict=${s.verdict} | score=${s.score}`
      + (s.language ? ` | lang=${s.language}` : "")
      + (p ? `\n   code: ${p.slice(0, 300)}` : "");
  }).join("\n");
  return base + header + scoreLine + weakLine + rows;
}
