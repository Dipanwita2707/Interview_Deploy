...
interface UseVoiceOptions {
  ...
  onInterrupt?: () => void;
  /** When true, applies more aggressive noise suppression to the mic stream */
...
export function useVoice({
  ...
  const sessionStartedAtRef = useRef<number | null>(null); // wall-clock ms when session became ready
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        audio: {
          ...
          // Advanced constraints honoured by Chrome/Edge when noise cancellation is on
          ...(noiseCancellationEnabled
            ? ({ googNoiseSuppression: true, googHighpassFilter: true } as MediaTrackConstraints)
            : {}),
        },
      });
      ...
      // AudioContext can start suspended in some browsers; resume explicitly.
      ...
      processor.onaudioprocess = (event) => {
        ...
        if (noiseCancellationEnabled && rms < NOISE_GATE_RMS_THRESHOLD) {
        ...
        // Software noise gate: when enhanced noise cancellation is on,
        // discard frames that are below the ambient noise floor to prevent
        // the relay from treating room noise as speech.
        const noiseGateThreshold = noiseCancellationEnabled ? 0.008 : 0.0;
        if (noiseGateThreshold > 0 && rms < noiseGateThreshold) {
          return;
        }
  ...
  }, [noiseCancellationEnabled, onError]);
  ...
  const saveAndComplete = useCallback(async () => {
    ...
    const actualDurationSeconds = sessionStartedAtRef.current
