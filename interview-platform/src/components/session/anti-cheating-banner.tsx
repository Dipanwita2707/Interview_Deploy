...
export function AntiCheatingGuard({ enabled, sessionId }: AntiCheatingGuardProps) {
  ...
  const lastPersistedViolation = useRef<Map<string, number>>(new Map());
  ...
  const persistViolation = useCallback(
    (violation: AntiCheatingViolation) => {
      ...
      const key = `${violation.type}:${violation.detail ?? ""}`;
      const previousTs = lastPersistedViolation.current.get(key) ?? 0;
      const dedupeWindowMs = violation.type === "page_departure" ? 1500 : 3000;
      if (violation.timestamp - previousTs < dedupeWindowMs) {
        return;
      }
      lastPersistedViolation.current.set(key, violation.timestamp);
