...
function normalizeMicConfirmationText(text: string): string {
  return text
    .toLowerCase()
...
function MicCheck({ done, onDone, language, allowSkip = true }: { done: boolean; onDone: () => void; language?: string; allowSkip?: boolean }) {
  ...
  const isAffirmativeMicResponse = useCallback((text: string) => {
    const normalized = normalizeMicConfirmationText(text);

    if (!normalized) {
      return false;
    }

    return getPositiveWords(languageRef.current).some((phrase) => {
      const candidate = normalizeMicConfirmationText(phrase);
  ...
  const analyzeResponse = useCallback((text: string) => {
    ...
    if (trimmed.length > 0 && isAffirmativeMicResponse(trimmed)) {
      setPhase("idle");
      onDoneRef.current();
      return;
    }
    setPhase("confirm");
  }, [isAffirmativeMicResponse]);
  ...
  const cleanupMicCapture = useCallback(() => {
    if (micCtxRef.current) {
      micCtxRef.current.close().catch(() => {});
      micCtxRef.current = null;
    }
  ...
  const getBrowserSpeechRecognition = useCallback((): BrowserSpeechRecognitionConstructor | null => {
  ...
  const startListening = useCallback(() => {
    ...
    let browserRecognitionEnded = false;
    ...
    if (SpeechRecognitionApi) {
      try {
        ...
        recognition.lang = getSpeechSynthesisLocale(languageRef.current);
        ...
        recognition.onresult = (event) => {
          ...
          lastAsrText = heard.trim();
          if (lastAsrText) {
            setTranscript(lastAsrText);
            setPhase("listening");
          }
          if (isAffirmativeMicResponse(lastAsrText)) {
            finish(lastAsrText);
          }
        };
        recognition.onerror = () => {
          browserRecognitionEnded = true;
        };
        recognition.onend = () => {
          browserRecognitionEnded = true;
          if (!handled && lastAsrText) {
            finish(lastAsrText);
          }
        };
        recognition.start();
    ...
    const connector = new RelayConnector<Record<string, unknown>>({
      ...
      onPermanentFailure: () => {
        if (!handled && (lastAsrText || browserRecognitionEnded)) {
          finish(lastAsrText);
        }
      },
    });
  ...
  }, [analyzeResponse, cleanupMicCapture, getBrowserSpeechRecognition, isAffirmativeMicResponse, stopTtsPlayback]);
        ...
        <div className="flex-1 space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {phase === "confirm" && !done && allowSkip &&
              "We heard audio, but not a clear confirmation. Please say \"yes\" or \"I can hear you\" and try again, or "}
            {phase === "confirm" && !done && allowSkip && (
              <button type="button" className="font-medium text-primary hover:underline" onClick={() => setShowSkipDialog(true)}>
                skip this step
              </button>
            ...
            {phase === "confirm" && !done && allowSkip && "."}
            {phase === "confirm" && !done && !allowSkip &&
              "We heard audio, but not a clear confirmation. Please say \"yes\" or \"I can hear you\" and try again."}
            {done &&
  ...
  const header = (
    <header className="sticky top-0 z-50 flex h-14 items-center border-b bg-card px-4 sm:px-6">
      <div className="flex items-center gap-1">
        ...
        <span className="font-heading text-sm font-bold tracking-[1px]">SMART Interview</span>
      </div>
    </header>
  );
