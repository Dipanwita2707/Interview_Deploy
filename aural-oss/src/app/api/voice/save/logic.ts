export type VoiceSavePayload = {
  ...
  /** Actual wall-clock duration in seconds, tracked by the client from session start */
...
export async function handleVoiceSave(
  ...
  const { sessionId, messages, complete, currentQuestionIndex, actualDurationSeconds } = payload;
  ...
  try {
    ...
    if (complete) {
      ...
      if (session) {
        ...
        const now = ops.now();
        // Prefer client-measured wall-clock duration (most accurate);
        // fall back to server-computed message-timestamp delta.
        const computedDuration = Math.round((now.getTime() - actualStart) / 1000);
        const duration = (typeof actualDurationSeconds === "number" && actualDurationSeconds > 0)
