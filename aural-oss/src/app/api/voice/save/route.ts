...
async function generateSummary(
  ...
  try {
    ...
    // Do not generate a summary/score if the candidate never spoke
    const userMessages = allMessages.filter((m) => m.role === "USER" || m.role === "user");
    if (userMessages.length === 0) {
      log.info(`Session ${sessionId}: no user messages — skipping summary to avoid hallucinated scores`);
      return;
    }
    ...
    for (const field of [
      ...
      if (parsed[field] !== undefined) {
        insightsData[field] = parsed[field];
      }
    }
