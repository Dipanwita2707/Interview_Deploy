...
export default function SettingsTab() {
  ...
  const [noiseCancellationEnabled, setNoiseCancellationEnabled] = useState<boolean>(
    (interview as any).noiseCancellationEnabled ?? false,
  );
  const [recordingConfig, setRecordingConfig] = useState<RecordingConfig>(
    (() => {
      const raw = (interview as any).recordingConfig;
      if (raw && typeof raw === "object") return { ...DEFAULT_RECORDING_CONFIG, ...raw };
          ...
          <div className="rounded-lg border p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <Label>Enable Noise Cancellation</Label>
                  <p className="text-xs text-muted-foreground">
                    Applies enhanced browser noise suppression and a software noise gate to the candidate&apos;s microphone during the interview
                  </p>
                </div>
              </div>
              <Switch
            ...
            {noiseCancellationEnabled && (
              <div className="mt-3 rounded-md bg-blue-50 px-3 py-2 text-xs text-blue-800 dark:bg-blue-950/30 dark:text-blue-200">
                <p className="font-medium">When enabled:</p>
                <ul className="mt-1 list-inside list-disc space-y-0.5">
                  <li>Browser-level noise suppression and echo cancellation are maximised</li>
                  <li>A software noise gate suppresses ambient room noise below audible speech</li>
                  <li>Works best in Chrome and Edge; Safari uses its own audio processing</li>
                </ul>
              </div>
            )}
          </div>
      ...
      <Card className="md:col-span-2">
        ...
        <CardContent>
          <p className="mb-3 text-xs text-muted-foreground">
            Choose what is captured during a voice interview. Requires <strong>Video</strong> mode to be enabled above.
          </p>
          <div className="space-y-2 rounded-lg border p-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Record combined mic + AI audio for the session</p>
                </div>
              </div>
              <Switch
                checked={recordingConfig.voiceRecording}
                disabled={!voiceEnabled}
                onCheckedChange={(v) => setRecordingConfig((c) => ({ ...c, voiceRecording: v }))}
              />
            </div>
            <div className="border-t" />
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Capture candidate camera as a video file</p>
                </div>
              </div>
              <Switch
                checked={recordingConfig.videoRecording}
                disabled={!videoEnabled}
                onCheckedChange={(v) => setRecordingConfig((c) => ({ ...c, videoRecording: v }))}
              />
            </div>
            <div className="border-t" />
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Capture candidate screen share as a video file</p>
                </div>
              </div>
              <Switch
                checked={recordingConfig.screenRecording}
                disabled={!videoEnabled}
                onCheckedChange={(v) => setRecordingConfig((c) => ({ ...c, screenRecording: v }))}
              />
            </div>
            <div className="border-t" />
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <Label>Camera Snapshots</Label>
                  <p className="text-xs text-muted-foreground">Periodically capture screenshots from the candidate&apos;s camera</p>
                </div>
              </div>
              <Switch
                checked={recordingConfig.cameraSnapshot}
                disabled={!videoEnabled}
                onCheckedChange={(v) => setRecordingConfig((c) => ({ ...c, cameraSnapshot: v }))}
              />
            </div>
            <div className="border-t" />
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <Label>Screen Snapshots</Label>
                  <p className="text-xs text-muted-foreground">Periodically capture screenshots from the candidate&apos;s screen share</p>
                </div>
              </div>
              <Switch
                checked={recordingConfig.screenSnapshot}
                disabled={!videoEnabled}
                onCheckedChange={(v) => setRecordingConfig((c) => ({ ...c, screenSnapshot: v }))}
              />
            </div>
          </div>
          {!videoEnabled && (
            <p className="mt-2 text-xs text-amber-600 dark:text-amber-400">
              Enable <strong>Video</strong> mode above to unlock recording options.
            </p>
          )}
        </CardContent>
      </Card>
