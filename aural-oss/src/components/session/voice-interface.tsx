...
interface VoiceInterfaceProps {
  ...
  onComplete?: () => void;
  videoMode?: boolean;
  /** Granular recording options (which streams/snapshots to capture) */
  recordingConfig?: RecordingConfig;
  /** Enable enhanced noise cancellation on the mic stream */
    ...
    try {
      ...
      try {
        ...
        if (videoMode && recording.isRecording) {
          ...
          const recordingPayload: Record<string, unknown> = { sessionId };
          if (result.audioUrl) {
            recordingPayload.audioRecordingUrl = result.audioUrl;
          }
          if (result.audioDuration !== undefined) {
            recordingPayload.audioDuration = result.audioDuration;
          }
          await withTimeout(
