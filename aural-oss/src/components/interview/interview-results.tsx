...
export function InterviewResults({
  ...
  const insights = trpc.analysis.getInterviewInsights.useQuery(
    { interviewId },
    {
      staleTime: 30_000,
      refetchOnWindowFocus: false,
...
function SessionDetail({
  ...
  const insightsData = summary.data?.insights as
    ...
    | string[]
    | null;
  const executiveSummary = Array.isArray(insightsData)
    ? null
    : (insightsData?.executiveSummary ?? null);
  const scoreBreakdown = Array.isArray(insightsData)
    ? []
    : (insightsData?.scoreBreakdown ?? []);
  const skillMap = Array.isArray(insightsData)
    ? []
    : (insightsData?.skillMap ?? []);
  const roadmap = Array.isArray(insightsData)
    ? null
    : (insightsData?.roadmap ?? null);
  const keyInsights = Array.isArray(insightsData)
    ? insightsData
    : (insightsData?.keyInsights ?? []);
  const strengths = Array.isArray(insightsData)
    ? []
    : (insightsData?.strengths ?? []);
  const risks = Array.isArray(insightsData)
    ? []
    : (insightsData?.risks ?? []);
  const followUpQuestions = Array.isArray(insightsData)
    ? []
    : (insightsData?.followUpQuestions ?? []);
  const notableQuotes = Array.isArray(insightsData)
    ? []
    : (insightsData?.notableQuotes ?? []);
  ...
  const authenticityCounts = antiCheatingLog.reduce<Record<string, number>>(
    (acc, event) => {
  ...
  const trustPenalty =
    (authenticityCounts.page_departure ?? 0) * 4 +
    (authenticityCounts.tab_switch ?? 0) * 4 +
    (authenticityCounts.focus_lost ?? 0) * 3 +
    (authenticityCounts.multi_screen ?? 0) * 12 +
    (authenticityCounts.paste ?? 0) * 8 +
    (authenticityCounts.copy ?? 0) * 3 +
    (authenticityCounts.cut ?? 0) * 3;
  const trustScore = Math.max(0, Math.min(100, 100 - trustPenalty));
  const participantMetadata =
    (summary.data?.participantMetadata as Record<string, unknown> | null) ?? null;
      ...
      {summary.isLoading ? (
        ...
        <>
          ...
          >
              ...
              return (
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col items-stretch gap-4 sm:flex-row">
                      ...
                      <CardContent className="flex h-full items-center gap-5 p-5">
                        ...
                        <div className="min-w-0 flex-1 space-y-1.5">
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                            {summary.data?.participantEmail && (
                              <span>{summary.data.participantEmail}</span>
                            )}
                            {summary.data?.participantPhone && (
                              <span>{summary.data.participantPhone}</span>
                            )}
                    ...
                    <div className="grid gap-4 sm:w-80 sm:grid-cols-2">
                      {/* Score card */}
                      {avgQuestionScore && (
                        <Card>
                          <CardContent className="flex h-full flex-col items-center justify-center p-5">
                            ...
                            <p className="mt-2 text-3xl font-bold">
                              {Math.round(parseFloat(avgQuestionScore) * 10)}%
                            </p>
                      ...
                      <Card>
                        <CardContent className="flex h-full flex-col items-center justify-center p-5">
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {antiCheatingLog.length} integrity events
                          </p>
                        </CardContent>
                      </Card>
                    </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-4">
                    {[
                      ["Tab / Page Departures", (authenticityCounts.page_departure ?? 0) + (authenticityCounts.tab_switch ?? 0)],
                      ["Focus Lost", authenticityCounts.focus_lost ?? 0],
                      ["Paste Events", authenticityCounts.paste ?? 0],
                      ["Multi-Screen", authenticityCounts.multi_screen ?? 0],
                    ].map(([label, value]) => (
                  ...
                  <CardTitle className="flex items-center gap-2">
                    ...
                    {executiveSummary.overallVerdict && (
                      <Badge variant="outline" className="ml-auto capitalize">
                        {executiveSummary.overallVerdict.replace(/_/g, " ")}
                      </Badge>
                    )}
                  </CardTitle>
                </CardHeader>
                      ...
                      <tbody>
                        {skillMap.map((item, i) => (
                          <tr key={i} className="border-b last:border-0">
                            <td className="py-3 pr-3 font-medium">{item.skill}</td>
                            ...
                            <td className="py-3 pr-3">
                              <Badge
                                ...
                                className={cn(
                                  ...
                                  (item.status === "solid" || item.status === "strength") &&
                ...
                <CardContent className="grid gap-4 md:grid-cols-2">
                  {scoreBreakdown.map((item, i) => (
                    <div key={i} className="space-y-2 rounded-lg border p-4">
                      ...
                      <Progress value={(item.score || 0) * 10} className="h-2" />
                ...
                <CardContent className="space-y-6">
                  {questionEvaluations.map((qe, i) => (
                    <div key={i} className="space-y-3 rounded-lg border p-4">
                      ...
                      {qe.evidence && qe.evidence.length > 0 && (
                        <div>
                          <p className="mb-1 text-xs font-medium">
                            Evidence
                          </p>
                          <ul className="list-disc space-y-0.5 pl-4 text-xs text-muted-foreground">
                            {qe.evidence.map((item, j) => (
            ...
            {roadmap && ((roadmap.items && roadmap.items.length > 0) || roadmap.startHere) && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Sparkles className="h-4 w-4" />
                    {roadmap.title || "Personalized Placement Roadmap"}
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-5">
                  ...
                  {roadmap.items && roadmap.items.length > 0 && (
                    <div className="space-y-4">
                      {roadmap.items
                        ...
                        .map((item) => (
                          <div key={`${item.priority}-${item.title}`} className="rounded-lg border p-4">
                            <div className="flex items-start gap-3">
                              ...
                              <div className="min-w-0 flex-1 space-y-2">
                                ...
                                <div className="grid gap-3 sm:grid-cols-2">
                                  <div className="rounded-md bg-muted/50 p-3">
                                    <p className="mb-1 text-xs font-medium">Goal</p>
                                    <p className="text-xs text-muted-foreground">{item.goal}</p>
                                  </div>
                                  <div className="rounded-md bg-muted/50 p-3">
                                    <p className="mb-1 text-xs font-medium">Practice Task</p>
                                    <p className="text-xs text-muted-foreground">{item.practiceTask}</p>
                                  </div>
                                </div>
                              </div>
                            </div>
                  ...
                  {roadmap.startHere && (
                    <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
                      ...
                      <p className="text-sm leading-relaxed">{roadmap.startHere}</p>
                    </div>
                  )}
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-4">
                  {criteriaEvaluations.map((ce, i) => (
                    <div key={i} className="space-y-2 rounded-lg border p-4">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        {ce.strengths && ce.strengths.length > 0 && (
                          <div>
                            <p className="mb-1 text-xs font-medium text-secondary-600 dark:text-secondary-400">
                              Strengths
                            </p>
                            <ul className="list-disc space-y-0.5 pl-4 text-xs text-muted-foreground">
                              {ce.strengths.map((item, j) => (
                                <li key={j}>{String(item)}</li>
                              ))}
                            </ul>
                          </div>
                        )}
                        {ce.risks && ce.risks.length > 0 && (
                          <div>
                            <p className="mb-1 text-xs font-medium text-tertiary-600 dark:text-tertiary-300">
                              Risks
                            </p>
            ...
            {/* Video Recordings (camera + screen) */}
            {(() => {
              const artifacts = summary.data?.recordingArtifacts as
                | {
                    url: string;
                    path: string;
                    timestamp: string;
                ...
                | null
                | undefined;
              const videoArtifacts = artifacts?.filter(
                (a) => a.type === "camera_video" || a.type === "screen_video",
              );
              if (!videoArtifacts || videoArtifacts.length === 0) return null;
              const cameraVids = videoArtifacts.filter((a) => a.type === "camera_video");
              const screenVids = videoArtifacts.filter((a) => a.type === "screen_video");
              return (
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      <Badge variant="secondary" className="ml-1 font-normal">
                        ...
                        {videoArtifacts.length === 1 ? "recording" : "recordings"}
                      </Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6">
